            ...
            for (SortedMap.Entry e : map.entrySet()) {
                ...
                for (int i = 0; i < megtett_utak.size(); i++) 
                    for (int j = i + 1; j < megtett_utak.size(); j++) 
                        if (megtett_utak.get(i).getSzakasz_egyik_vege().equals(megtett_utak.get(j).getSzakasz_masik_vege())
                          || megtett_utak.get(j).getSzakasz_egyik_vege().equals(megtett_utak.get(i).getSzakasz_masik_vege())
                          || megtett_utak.get(i).getSzakasz_egyik_vege().equals(megtett_utak.get(j).getSzakasz_egyik_vege())
                          ||  megtett_utak.get(i).getSzakasz_masik_vege().equals(megtett_utak.get(j).getSzakasz_masik_vege()))
                            darab++; 
                
                if (darab == megtett_utak.size() - 1)
                    összefüggő = true;
                else összefüggő = false; 
