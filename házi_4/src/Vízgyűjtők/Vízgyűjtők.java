...
public class Vízgyűjtők {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            // a listán belüli gyűjtők listaba is beteszem a vízgyűjtőt
                ...
                for (int j = 0; j < adatok.size(); j++) {
                    if(adatok.get(j).contains(";")) {
                        ...
                        if(i.getGyűjtők().contains(token[1]) && !i.getGyűjtők().contains(token[0])) {
                                ...
                                volt = true;
                                j = -1; // így mindig előröl ellenőrzi a belső for ciklust 
                        }
                    }
                }
                
                // ha a volt false marad
                // akkor lép ki, ha végigment a belső cikluson, de egy elemet sem talált
                if(!volt) break;
            }
        }
        for(Vízgyűjtő i : lista)
